...
export default function SourceDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await authFetch(`/api/sources/${params.id}`);
        if (res.status === 401) { router.push("/login"); return; }
        if (!res.ok) throw new Error(`Source fetch failed: ${res.status}`);
        const data = await res.json();
        setSource(data);
        if (data.preview && Array.isArray(data.preview) && data.preview.length > 0) {
          // API returns: [{ headers: [...] }, { row1 }, { row2 }, ...]
          const headers = data.preview[0]?.headers || [];
          const rows = data.preview.slice(1);
          setPreview({ columns: headers, rows, totalRows: data.rowsCount || rows.length });
        } else {
