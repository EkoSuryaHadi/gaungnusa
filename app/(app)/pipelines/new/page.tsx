...
const STEP_TYPES = [
  ...
  { type: "SILVER_QUALITY", icon: "✨", label: "Silver Quality", color: "gold" },
...
const defaultConfig: Record<StepType, ConfigState> = {
  ...
  SILVER_QUALITY: { silverMode: "full", silverDomain: "", silverModules: "" },
        ...
        <div className="space-y-3">
          ...
          >
            <span style={{ fontSize: "11px", color: "#D4A853", lineHeight: "1.5" }}>
              AI-powered data profiling, classification, validation, quality scoring, and recommendations.
              Runs profiling → data type detection → duplicate/missing/outlier handling → validation → scoring.
            </span>
          </div>
          <Field label="Mode">
            <select
              value={config.silverMode ?? "full"}
              onChange={(e) => set("silverMode", e.target.value)}
              className="input"
            >
              <option value="quick">Quick — Profile + Validate + Score</option>
              <option value="full">Full — Full pipeline + Recommendations</option>
              <option value="deep">Deep — Full + Anomaly Detection</option>
              <option value="custom">Custom — Choose modules manually</option>
            </select>
          </Field>
          {(config.silverMode ?? "full") === "custom" && (
            <Field label="Modules" helper="Comma-separated: profiling,validation,scoring">
              <input
                value={config.silverModules ?? ""}
                onChange={(e) => set("silverModules", e.target.value)}
                placeholder="profiling,validation,scoring"
                className="input"
              />
            </Field>
          )}
          <Field label="Domain (optional)" helper="Auto-detect if empty. Options: iot, finance, sales, erp, hr">
            <select
              value={config.silverDomain ?? ""}
              onChange={(e) => set("silverDomain", e.target.value)}
              className="input"
            >
